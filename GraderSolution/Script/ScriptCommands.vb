...
Public Class ScriptCommands
   ...
   End Sub
    ...
    Public Sub checkCentered(ByVal formName As String, ByVal controlName As String, ByVal center As String, ByVal scoreAmount As String)
   ...
   End Sub
    ...
    'right a control is compared to other controls of the same type, the higher its x-value. Y values only go up 
    'when there is more than one control at the same x value(but probably a different y value). When there are 
    'multiple controls with the same(or close to the same) x value, the y values get assigned increasing from the
    'top control to the bottom. Here is an example of a screen. I will explain how to find the textboxes:
    '   ________________________
    '   |                       |   T = Textbox     Start by just looking for the specific control type you are looking for(in this case     
    '   |   L     T      T      |   L = Label       textboxes). The system differentials between control types so you don't need to worry 
    '   |   T     T             |                   about the labels. So start by looking at the farthest left textbox. There is only one
    '   |   L     T             |                   there, so its (x,y) coordinates are "#Point 1 1" since it is the highest textbox in 
    '   |                       |                   the farthest left textbox column. Then the next column farthest to the left is the
    '   |_______________________|                   middle column of textboxes. The top textbox will be in the first position so the 
    '                                               top-middle textbox is "#Point 2 1". The textbox right below that will be "#Point 2 2".
    '   The textbox right below that(in the bottom-middle position will be "#Point 2 3". The last textbox(top-right) is in the 3rd textbox
    '   column and is in the first(top) position so its point is "#Point 3 1".
    ...
    ' [location] In - The location of Form Control based off the reference to other controls in a grid as explained above.
    Public Sub find(ByVal controlType As String, ByVal controlName As String)
        ...
        If Not controlExists Then
            ...
            Dim errorMessage As String = "Control: " & controlName & " of type " & controlType & " could not be found"
    ...
    Public Sub loadForm(ByVal formName As String)
        ...
        If Not foundForm Then
            ...
            Dim errorMessage As String = "Could not find form: " & formName
